...
describe 'Puppet::Type.type(:xdmod_appkernel_setting)' do
  let(:xdmod_appkernel_setting) do
    Puppet::Type.type(:xdmod_appkernel_setting).new(name: 'vars/foo', value: 'bar')
  end
  ...
  it 'does not expect a name with whitespace' do
    ...
    }.to raise_error(Puppet::Error, %r{Invalid xdmod_appkernel_setting})
  end
  ...
  it 'accepts a valid value' do
    ...
    expect(xdmod_appkernel_setting[:value]).to eq '"bar"'
  end
  it 'does not accept a value with whitespace' do
    xdmod_appkernel_setting[:value] = 'b ar'
    expect(xdmod_appkernel_setting[:value]).to eq '"b ar"'
  end
  it 'accepts valid ensure values' do
    xdmod_appkernel_setting[:ensure] = :present
    expect(xdmod_appkernel_setting[:ensure]).to eq :present
    xdmod_appkernel_setting[:ensure] = :absent
    expect(xdmod_appkernel_setting[:ensure]).to eq :absent
  end
  ...
  describe 'autorequire File resources' do
    it 'autorequires /etc/xdmod/portal_settings.d/appkernels.ini' do
      ...
      expect(rel.target.ref).to eq xdmod_appkernel_setting.ref
    end
  end
end
